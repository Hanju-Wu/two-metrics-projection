...
%% Methods
[W1,H1,iter_PNCG,time_PNCG,x_PNCG,y_PNCG] = PNCG(V,Winit,Hinit,1e-4,100,1000);
%%
[W2,H2,iter_alspgrad,time_alspgrad,x_pgrad,y_pgrad] = pgrad(V,Winit,Hinit,1e-4,100,1000);
%%
[W3,H3,iter_pnm,time_pnm,x_pnm,y_pnm] = pnm(V,Winit,Hinit,1e-4,100,1000);
%% Plot
subplot(2,2,1)
semilogy(x_PNCG,y_PNCG,'Marker','o','MarkerIndices',[1:20:length(x_PNCG),length(x_PNCG)]);
hold on
semilogy(x_pgrad,y_pgrad,'Marker','*','MarkerIndices',[1:20:length(x_pgrad),length(x_pgrad)]);
semilogy(x_pnm,y_pnm,'Marker','diamond','MarkerIndices',[1:3:length(x_pnm),length(x_pnm)]);
hold off
lgd = legend('PNCG','pgrad','pnm');
xlabel('Time(s)')
